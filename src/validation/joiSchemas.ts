...
export const CorrectShiftDtoSchema = Joi.object({
    table_num_crew: Joi.string().min(1).required(),
    table_num_mng: Joi.string().min(1).required(),
    start: Joi.string().pattern(/^\d{2}\/\d{2}\/\d{4} \d{2}:\d{2}$/).required(),
    finish:Joi.string().pattern(/^\d{2}\/\d{2}\/\d{4} \d{2}:\d{2}$/).required(),
